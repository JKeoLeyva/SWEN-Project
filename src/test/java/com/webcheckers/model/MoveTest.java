...
class MoveTest {
    ...
    void spaceIsTaken() {
        for(int col = 0; col < Board.BOARD_SIZE; col+=2) {
            game.getBoard().setPiece(3, col, new Piece(Piece.Type.SINGLE, Piece.Color.WHITE));
            game.getBoard().setPiece(4, col + 1, new Piece(Piece.Type.SINGLE, Piece.Color.RED));
        }
        for(int col = 0; col < Board.BOARD_SIZE; col+=2){
            Position whitePiece = new Position(3, col);
            Position redPiece = new Position(4, col+1);
            CuT = new Move(whitePiece, redPiece);
            Message result = CuT.isValid(game.getBoard());
            assertEquals(result, TAKEN_SPACE_MESSAGE);
            CuT = new Move(redPiece, whitePiece);
            result = CuT.isValid(game.getBoard());
            assertEquals(result, TAKEN_SPACE_MESSAGE);
        }
    }
